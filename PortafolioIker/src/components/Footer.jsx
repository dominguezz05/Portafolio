...
function Footer() {
  ...
  const motivationalQuoteColor = isDarkMode
    ? "text-slate-500"
    : "text-slate-400"; // Color para la cita motivacional

  return (
    <footer className={`py-8 transition-colors duration-500 ${footerClasses}`}>
      <div className="max-w-4xl mx-auto text-center space-y-4 px-4">
        ...
        <p className={`text-lg font-semibold ${titleColor}`}>
          <AnimatedText keyProp={`footer-title-${language}`}>
            © {new Date().getFullYear()} Iker Domínguez -{" "}
            {language === "es"
        ...
        <nav className="flex justify-center flex-wrap gap-x-6 gap-y-2 text-sm">
          ...
          <a href="#certifications" className={`${linkHoverColor} transition`}>
            <AnimatedText keyProp={`footer-nav-certifications-${language}`}>
        ...
        <div className="flex justify-center gap-6 mt-4">
          <a
            ...
            aria-label={
              language === "es"
                ? "Perfil de GitHub de Iker Domínguez"
                : "Iker Domínguez's GitHub Profile"
            }
          ...
          <a
            ...
            aria-label={
              language === "es"
                ? "Perfil de LinkedIn de Iker Domínguez"
                : "Iker Domínguez's LinkedIn Profile"
            }
            className={`${linkHoverColor} transition`}
          >
