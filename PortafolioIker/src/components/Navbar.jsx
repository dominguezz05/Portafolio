...
function Navbar() {
  ...
  const sectionIds = useMemo(
    () => navLinksDefinition.map((link) => link.id),
    [],
  );
  ...
  const linkClass = (sectionId, isMobile = false) => {
    const baseLinkColor =
      (scrolled && isDarkMode && !isMobile) || (!scrolled && !isMobile)
        ? "text-white"
        : "text-gray-700";
    ...
    const mobileBaseLinkColor = isDarkMode
      ? "text-slate-200"
      : "text-slate-700";
    const mobileHoverColor = isDarkMode
      ? "hover:text-sky-400"
      : "hover:text-sky-600";
  ...
  const handleKeyDown = (event, href) => {
    if (event.key === "Enter" || event.key === " ") {
      ...
      handleMobileLinkClick(event, href);
    }
  };

  // Escape key handler for mobile menu
  useEffect(() => {
    // Eliminamos ": KeyboardEvent"
    ...
    <>
      ...
      >
        ...
        <div className="text-xl font-bold">
          <a
            ...
            aria-label={currentTexts.navAbout || "Ir al inicio"}
            className="focus:outline-none focus:ring-2 focus:ring-sky-500 rounded-lg"
          >
            <img
              src={currentLogo}
              alt="Logo IDC - Iker Domínguez Carrasco"
        ...
        <nav
          className="hidden md:flex flex-grow justify-center items-center gap-2 md:gap-4 text-sm md:text-base"
        ...
        >
          {navLinksDefinition.map((link) => (
            <a
              ...
              aria-label={currentTexts[link.textKey]}
              role="menuitem"
            >
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <button
            ...
            aria-label={
              isDarkMode
                ? currentTexts.toggleToLight
                : currentTexts.toggleToDark
            }
          ...
          <button
            ...
            aria-label={
              language === "es"
                ? currentTexts.toggleLangToEn
                : currentTexts.toggleLangToEs
            }
          ...
          >
            <FaGlobe
              className={`w-3.5 h-3.5 md:w-4 md:h-4 transition-colors duration-300 ${language === "es" ? "text-red-500" : "text-blue-500"}`}
            />
          ...
          <button
            ...
            aria-label={
              isMobileMenuOpen ? currentTexts.closeMenu : currentTexts.openMenu
            }
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            <div className="w-full max-w-xs mx-auto py-8 space-y-4" role="menu">
              {navLinksDefinition.map((link) => (
                <a
                  key={link.id}
                  href={link.href}
